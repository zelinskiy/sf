...
Fixpoint alternate' (fuel : nat) (l1 l2 : natlist) : natlist :=
  match fuel with
  | O => l2
  | S fuel' => match l2 with
               | nil => l1
               | h :: t => (hd O l1) :: h :: (alternate' fuel' (tl l1) t)
               end
  end.

Definition alternate (l1 l2 : natlist) : natlist :=
  alternate' (length l1) l1 l2.


Example test_alternate1:
  alternate [1;2;3] [4;5;6] = [1;4;2;5;3;6].
Proof. unfold alternate. simpl. reflexivity. Qed.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  ...
  | nil => true
  | x :: xs => if member x s2
               then subset xs (remove_all x s2)
               else false
  end.
...
(* None ? Consider 2 cases. We can construct baz using one of its constructors. For the first case (Baz1) we would need another constructor, so we go to second case (Baz2) which is simply a bit more sophisticated version of the first one. *)

(* Footnote : (A -> A) -> A implies Pierce's law *)
