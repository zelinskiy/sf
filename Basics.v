...
Axiom twiceZero : forall b, T b = Z.

Definition nat_to_TF := fun n => match n with | 0 => False | _ => True end.
...
Lemma twice_lemma_weak : forall b, nat_to_bin (b + b) = T (nat_to_bin b).
Proof.
  intros.
  induction b.
  simpl.
...
Lemma twice_lemma : forall b, nat_to_bin (bin_to_nat b + bin_to_nat b) = T b.
Proof.
  intros.
  induction b.
  simpl.
  pose (twiceZero Z) as H.
  symmetry in H.
  exact H.
  simpl.
  rewrite <- (plus_n_O (bin_to_nat b)).
  rewrite <- IHb.
  rewrite twice_lemma_weak.
  reflexivity.
  rewrite twice_lemma_weak.
  apply f_equal with (f := T).
  simpl.
  rewrite twice_lemma_weak  in IHb.
  rewrite <- (plus_n_O (bin_to_nat b)).
  rewrite twice_lemma_weak.
  rewrite IHb.
  simpl.
