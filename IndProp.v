...
Proof.
  ...
  rewrite H2 in H1.
  exact (ev_ev__ev (double n) (m + p) H1 (ev_double n)).
Qed.
...
Inductive total_relation : nat -> nat -> Prop :=
  | tot_rel : forall n m, n = m \/ n <> m -> total_relation n m.
...
| em_rel : forall n m, n = m /\ n <> m -> empty_relation n m.
    ...
    induction m.
    {
      ...
      }
      {
        ...
        }
        {
          ...
          rewrite H.
          assert (forall x, R 0 x x) as R_id.
          {
            induction x.
            { exact c1. }
            { apply c3. exact IHx. }
...
| subnil   : forall l, subseq nil l
| subdrop  : forall x l1 l2, subseq l1 l2 -> subseq l1 (x :: l2)
| submatch : forall x l1 l2, subseq l1 l2 -> subseq (x :: l1) (x :: l2).

Theorem subseq_refl: forall A (l : list A), subseq l l.
Proof.
  intros.
  induction l.
  - apply subnil.
...
Lemma app_nil : forall T (xs ys : list T), xs ++ ys = [] -> xs = [] /\ ys = [].
Proof.
  intros.
  destruct xs.
  destruct ys.
  ...
  - simpl. 
    ...
    * intro.
      ...
      + apply IHl.
        {
          simpl in H.
          rewrite (proj2 (beq_nat_false_iff n x)) in H.
          {
            exact H.
          }
          {
