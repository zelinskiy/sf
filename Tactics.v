...
Proof.
  intros.
  assert (n = m).
  { exact (beq_nat_true n m H). }
  assert (m = p).
  { exact (beq_nat_true m p H0). }
  rewrite <- H1 in H0.
  exact H0.
Qed.
...
Definition existsb' {X : Type} (p : X -> bool) (xs : list X) : bool :=
  negb (forallb (fun z => negb (p z)) xs).
...
Proof.
  ...
  - simpl.
    ...
    apply f_equal with (f := fun z => p x || z) in IHxs.
    exact IHxs.
Qed.
